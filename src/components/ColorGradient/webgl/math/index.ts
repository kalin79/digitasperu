...
export function mix( a: number, b: number, m: number ){
  return a * (1.0-m) + b*m;
}


export function map( num: number, min1: number, max1: number, min2: number, max2: number ) {
...
export function remapProgress( p: number, s: number, e: number ){
  const r = e-s;
  return clamp01(p/r - s/r)
}


export function randomFloat (minValue: number, maxValue: number) {
...
export function normalizeDeltaAngle( angle: number, dest: number ){
  let   d0 = dest - angle;
  const d1 = d0 - PI2
  const d2 = d0 + PI2
...
export function mat3Lookat( mat3 : mat3, dir : vec3 ){
...
export function quatSlerp( out: quat, a: number[], i1: number, i2: number, t: number ) {
    // benchmarks:
    //    http://jsperf.com/quaternion-slerp-implementations

    let ax = a[i1+0], ay = a[i1+1], az = a[i1+2], aw = a[i1+3],
        bx = a[i2+0], by = a[i2+1], bz = a[i2+2], bw = a[i2+3];
